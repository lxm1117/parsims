...
void evaluateN( pRun *pRn )
{
  ...
  if( do_init ){
    ...
    sprintf( cmd, "/usr/bin/time -f \"%%e\" -o %s nrniv -nogui %s %s %s >> %s 2>&1", 
	     eTimef, pRn->hoc_main, hoc_asg, hoc_fit, elog ); 

    //sprintf( cmd, "time -o %s nrniv -nogui %s %s %s >> %s 2>&1", 
//	     eTimef, pRn->hoc_main, hoc_asg, hoc_fit, elog ); 
...
void evaluateN_old( pRun *pRn )
{
  ...
  if( do_init ){
    ...
    sprintf( cmd, "/usr/bin/time -f \"%%e\" -o %s nrniv %s %s %s >> %s 2>&1", 
	     eTimef, pRn->hoc_main, hoc_asg, hoc_fit, elog );
